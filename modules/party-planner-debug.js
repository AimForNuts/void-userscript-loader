(function () {
  ...
  function createPartyPlannerDebugModule(definition) {
    let appRef;
    ...
    function norm(s) { return clean(s).toLowerCase().replace(/['']/g, '').replace(/&amp;/g, '&').replace(/[^a-z0-9]+/g, ' ').trim(); }
    function slug(s) { return norm(s).replace(/\s+/g, '_'); }
    function esc(s) { return String(s ?? '').replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c])); }
    function text(el) { return clean(el?.textContent || ''); }
    function num(v) { let s = String(v ?? '').trim().replace(/,/g, ''); if (!s || s === '—') return 0; const mult = /k$/i.test(s) ? 1000 : /m$/i.test(s) ? 1000000 : /b$/i.test(s) ? 1000000000 : 1; s = s.replace(/[kmb]$/i, ''); const n = Number(s); return Number.isFinite(n) ? n * mult : 0; }
    ...
    function installCSS() {
      if (document.getElementById('vilCssPartyDebug')) return;
      ...
      const style = document.createElement('style'); style.id = 'vilCssPartyDebug'; style.textContent = css; document.head.appendChild(style);
    }
    ...
    return {
      ...
      destroy() {
        document.getElementById('vilCssPartyDebug')?.remove();
      },
    };
  }

  window.VoidIdleModules = window.VoidIdleModules || {};
  window.VoidIdleModules['party-planner-debug'] = createPartyPlannerDebugModule({
    id: 'party-planner-debug',
    name: 'Party Planner Debug',
    icon: '🔍',
    description: 'Inventory scan, party tools, rune planner, tool planner, crafting debug, and spirit essence trace.',
  });
})();
